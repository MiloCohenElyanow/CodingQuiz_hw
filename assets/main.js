...
function writeStorageToPage(){
  boardName.textContent = JSON.parse(localStorage.getItem("saveNameOne"));
  boardScore.textContent = JSON.parse(localStorage.getItem("saveScoreOne"));
}
...
const blockSize = 16;  // The size of each block in pixels
...
function drawWorld() {
  for (let y = 0; y < world.length; y++) {
    for (let x = 0; x < world[y].length; x++) {
      ...
      ctx.fillRect(x * blockSize, y * block
      }
    }
  }
*/

//here is half of minecraft coded in js in 8-bit